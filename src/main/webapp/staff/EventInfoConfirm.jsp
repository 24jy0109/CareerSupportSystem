...
<main>
    ...
    <form id="eventForm" action="event" method="post">
        ...
        <c:forEach var="g" items="${event.joinGraduates}">
            <input type="hidden" name="graduateStudents" value="${g.graduateStudentNumber}" />
        </c:forEach>

        <!-- command 用 hidden -->
        <input type="hidden" id="commandField" name="command" value="RegistEvent" />
        ...
        <div class="bottom-btn-split">
            ...
            <button type="submit" onclick="document.getElementById('commandField').value='RegistEventBack'">
                戻る
            </button>
            <button type="submit">
                確定
