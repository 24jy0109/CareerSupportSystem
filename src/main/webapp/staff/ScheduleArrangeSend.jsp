...
<%
    ...
    if (emailDTO != null) {
        ...
        mailBody = grad.getGraduateName() + " 様\n\n"
                 ...
                 + "担当スタッフまでお気軽にご連絡ください。\n\n"
                 + "よろしくお願いいたします。\n\n";
        defaultStaff = staffs.get(0); // デフォルトは卒業生担当者ではなく最初のスタッフ
    }
%>
...
<form action="event" method="post">
    ...
    <div class="schedulearrange-row">
        ...
        <div>
            <div class="toname-toemail"><%=grad.getGraduateName()%>（<%=grad.getGraduateStudentNumber()%>）</div>
            <input type="hidden" name="graduateStudentNumber" value="<%=grad.getGraduateStudentNumber()%>">
            <input type="hidden" name="companyId" value="<%=grad.getCompany().getCompanyId()%>">
        </div>
    </div>

    <div class="schedulearrange-row">
        <div class="field-name">件名</div>
        <input type="text" name="mailTitle" size="60" value="<%=mailTitle%>" class="email-title" required>
    </div>

    <div class="schedulearrange-row">
        <div class="field-name">本文</div>
        <textarea name="mailBody" class="email-text" rows="30" cols="100" required><%=mailBody%></textarea>
    </div>
